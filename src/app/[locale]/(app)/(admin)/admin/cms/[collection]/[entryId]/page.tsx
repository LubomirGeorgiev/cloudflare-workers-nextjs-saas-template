...
interface EditEntryPageProps {
  params: Promise<{ collection: string; entryId: string; locale: Locale }>;
}
...
export default async function EditEntryPage({ params }: EditEntryPageProps) {
  ...
  const { collection, entryId, locale } = await params;
  ...
  const entry = await getFreshCmsEntryById(entryId);
  if (!entry) {
    return redirect({ href: `/admin/cms/${collection}`, locale });
  }
