...
- A commit must NEVER contain multiple new migrations unless a human is explicitly asked and gives permission. Without that permission, consolidate before committing: delete the incremental migration files, regenerate a single migration from the final schema, and reset/re-migrate local dev database state so its journal matches the regenerated files. This rule covers migrations you authored; migrations arriving from an upstream template merge are exempt and follow "Merging Template Migrations Into a Fork" below.
