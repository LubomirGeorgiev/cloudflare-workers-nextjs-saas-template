...
- Public pages render on every request and are never stored in a shared cache: the root layout exports `dynamic = "force-dynamic"`, and no page may add `export const revalidate` or `export const dynamic = "force-static"`, because a shared-cache hit skips the Worker and with it the locale redirect. The data behind a page is cached in KV through `setCacheScope` — see [docs/edge-caching.md](docs/edge-caching.md).
...
- `src/proxy.ts` runs next-intl on every path `shouldLocalizePathname` accepts, and nothing else; a rule the edge can decide from the URL alone (the disabled-i18n prefix collapse, the OpenGraph cookie strip) lives in `worker-entrypoint.ts`. Its `config.matcher` only drops framework internals. Almost nothing belongs outside `app/[locale]/` now: only machine endpoints (`/api/*`, `/markdown/*`, `/llms.txt`) do, and each needs its segment in `NON_LOCALIZED_PATH_SEGMENTS` in `src/i18n/localized-paths.ts` — `localized-paths.test.ts` walks `src/app/` and fails if you forget.
